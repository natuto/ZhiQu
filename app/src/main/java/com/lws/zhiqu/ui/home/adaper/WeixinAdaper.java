...
public class WeixinAdaper extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    private static final int ITEM_FOOTER = 0;
    private static final int ITEM_NORMAL = 1;
    public  static final int LOAD_MORE = 2;
    public  static final int LOAD_PULL_TO = 3;
    ...
    public WeixinAdaper(Context context) {
        mContext = context;
    }

    public void addAll(WeiXinBean weiXinBean) {
        mWeiXinBean.getResult().getList().addAll(weiXinBean.getResult().getList());
        notifyDataSetChanged();
    }
    public void  replaceAll(WeiXinBean weiXinBean){
        if ( mWeiXinBean!=  null) {
            mWeiXinBean.getResult().getList().clear();
        }
        mWeiXinBean = weiXinBean;
        notifyDataSetChanged();
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        return new WeixinViewHolder(rootView);
        }
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        }else if(holder instanceof WeixinViewHolder){
            WeixinViewHolder weixinViewHolder = (WeixinViewHolder) holder;
            weixinViewHolder.bindItem(position);
        }
    ...
    public int getItemCount() {
        if (mWeiXinBean != null) {
            ...
            return mWeiXinBean.getResult().getList().size() + 1;
        }
        return  0;
    ...
    class WeixinViewHolder extends RecyclerView.ViewHolder{
        ...
        private void bindItem(final int position){
            ...
            mSource.setText(mWeiXinBean.getResult().getList().get(position).getSource());
            mTitle.setText(mWeiXinBean.getResult().getList().get(position).getTitle());
            Glide.with(mContext).load(mWeiXinBean.getResult().getList().get(position).getFirstImg()).crossFade().into(mImageView);
            mCardView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    mTitle.setTextColor(ContextCompat.getColor(App.getContext(),R.color.colorHint));
                    Intent intent = new Intent(mContext,HomeWeixinDetailActivity.class);

                    intent.putExtra("url",mWeiXinBean.getResult().getList().get(position).getUrl());
